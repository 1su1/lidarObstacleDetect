#ifndef NewPatchWork_H
#define NewPatchWork_H
...
template<typename PointT> class NewPatchWork
{
...
public:
    ...
    NewPatchWork(ros::NodeHandle* nh)
        : node_handle_(*nh)
    {
        // Init ROS related
...
private:
     ...
     */
    geometry_msgs::PolygonStamped set_polygons(int zone_idx, int r_idx, int theta_idx,
                                               int num_split);

    void set_ground_likelihood_estimation_status(const int zone_idx, const int ring_idx,
                                                 const int concentric_idx, const double z_vec,
                                                 const double z_elevation,
                                                 const double ground_flatness);
};

template<typename PointT>
inline void NewPatchWork<PointT>::initialize_zone(Zone& z, int num_sectors, int num_rings)
{
    z.clear();
...
template<typename PointT>
inline void NewPatchWork<PointT>::flush_patches_in_zone(Zone& patches, int num_sectors,
                                                        int num_rings)
{
    for (int i = 0; i < num_sectors; i++)
    {
...
template<typename PointT> inline void NewPatchWork<PointT>::flush_patches(vector<Zone>& czm)
{
...
template<typename PointT>
inline void NewPatchWork<PointT>::estimate_plane(const pcl::PointCloud<PointT>& ground)
{
...
template<typename PointT>
inline void NewPatchWork<PointT>::extract_initial_seeds(const int                      zone_idx,
...
template<typename PointT>
inline void NewPatchWork<PointT>::reflected_noise_removal(pcl::PointCloud<PointT>& cloud_in,
                                                          pcl::PointCloud<PointT>& cloud_nonground)
{
...
template<typename PointT>
inline void NewPatchWork<PointT>::estimate_ground(pcl::PointCloud<PointT>  cloud_in,
        ...
        for (int ring_idx = 0; ring_idx < num_rings_each_zone_[zone_idx]; ++ring_idx)
        {
            for (int sector_idx = 0; sector_idx < num_sectors_each_zone_[zone_idx]; ++sector_idx)
            {
                ...
                if (visualize_)
                {
                    ...
                    set_ground_likelihood_estimation_status(zone_idx,
                                                            ring_idx,
                                                            concentric_idx,
                                                            ground_uprightness,
                                                            ground_elevation,
...
template<typename PointT> inline void NewPatchWork<PointT>::update_elevation_thr(void)
{
...
template<typename PointT> inline void NewPatchWork<PointT>::update_flatness_thr(void)
{
    for (int i = 0; i < num_rings_of_interest_; i++)
    {
...
template<typename PointT>
inline void NewPatchWork<PointT>::temporal_ground_revert(
...
// For adaptive
template<typename PointT>
inline void NewPatchWork<PointT>::extract_piecewiseground(const int                      zone_idx,
...
template<typename PointT>
inline geometry_msgs::PolygonStamped NewPatchWork<PointT>::set_polygons(int zone_idx, int r_idx,
                                                                        int theta_idx,
                                                                        int num_split)
{
...
template<typename PointT>
inline void NewPatchWork<PointT>::set_ground_likelihood_estimation_status(
    const int zone_idx, const int ring_idx, const int concentric_idx, const double z_vec,
...
template<typename PointT>
inline void NewPatchWork<PointT>::calc_mean_stdev(std::vector<double> vec, double& mean,
                                                  double& stdev)
{
...
template<typename PointT>
inline double NewPatchWork<PointT>::xy2theta(const double& x, const double& y)
{   // 0 ~ 2 * PI
    // if (y >= 0) {
...
template<typename PointT>
inline double NewPatchWork<PointT>::xy2radius(const double& x, const double& y)
{
    return sqrt(pow(x, 2) + pow(y, 2));
}

template<typename PointT>
inline void NewPatchWork<PointT>::pc2czm(const pcl::PointCloud<PointT>& src, std::vector<Zone>& czm,
                                         pcl::PointCloud<PointT>& cloud_nonground)
{
