...
void PatchWork::downsample(const pcl::PointCloud<pcl::PointXYZI>::Ptr& in_cloud,
...
void PatchWork::extract_initial_seeds_(const int                              zone_idx,
...
void PatchWork::extract_piecewiseground(const int                              zone_idx,
...
void PatchWork::estimate_ground(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud_in,
...
// double &time_taken)
{
    ...
    for (int k = 0; k < num_zones_; ++k) {
        flush_patches_in_zone(
            ConcentricZoneModel_[k], num_sectors_each_zone_[k], num_rings_each_zone_[k]);
    }
            ...
            for (uint16_t sector_idx = 0; sector_idx < num_sectors_each_zone_[k]; ++sector_idx) {
                if (zone[ring_idx][sector_idx].points.size() > num_min_pts_) {
                    extract_piecewiseground(
                        k, zone[ring_idx][sector_idx], regionwise_ground_, regionwise_nonground_);
                        ...
                        if (concentric_idx < num_rings_of_interest_) {
                            if (ground_z_elevation > elevation_thr_[ring_idx + 2 * k]) {
                                if (flatness_thr_[ring_idx + 2 * k] > surface_variable) {
                                    if (verbose_) {
                                        // std::cout << "\033[1;36m[Flatness] Recovery operated.
                                        // Check "
                                        //           << ring_idx + 2 * k
                                        //           << "th param. flatness_thr_: " <<
                                        //           flatness_thr_[ring_idx + 2 * k]
                                        //           << " > "
                                    ...
                                    if (verbose_) {
                                        // std::cout << "\033[1;34m[Elevation] Rejection operated.
                                        // Check "
                                        //           << ring_idx + 2 * k
                                        //           << "th param. of elevation_thr_: " <<
                                        //           elevation_thr_[ring_idx + 2 * k]
                                        //           << " < "
                                        //           << ground_z_elevation << "\033[0m" <<
                                        //           std::endl;
                            ...
                            if (using_global_thr_ && (ground_z_elevation > global_elevation_thr_)) {
                                std::cout << "\033[1;33m[Global elevation] " << ground_z_elevation
