...
{
    visualization_msgs::MarkerArray label_markers, arrow_markers, centroid_markers, polygon_hulls,
        bounding_cubes, bounding_boxes, object_models;
        ...
        if (IsObjectValid(object) && (object.pose_reliable || object.label == "unknown") &&
            ...
            if (true) {
                ...
                float RT[12] = {
                    ...
                    2 * x * y - 2 * z * w,
                    2 * x * z + 2 * y * w,
                    float(object.pose.position.x),   // pose.position表示包围盒中心点坐标
                    2 * x * y + 2 * z * w,
                    1 - 2 * x * x - 2 * z * z,
                    2 * y * z - 2 * x * w,
                    float(object.pose.position.y),
                    2 * x * z - 2 * y * w,
                    2 * y * z + 2 * x * w,
                    1 - 2 * x * x - 2 * y * y,
    ...
    const autoware_msgs::DetectedObjectArray& in_objects)
{
    ...
    for (auto const& object : in_objects.objects) {
        if (IsObjectValid(object) && object.label != "unknown" &&
            ...
            if (object.pose_reliable) model.pose.orientation = object.pose.orientation;
...
bool VisualizeDetectedObjects::IsObjectValid(const autoware_msgs::DetectedObject& in_object)
{
    if (!in_object.valid || std::isnan(in_object.pose.orientation.x) ||
        std::isnan(in_object.pose.orientation.y) || std::isnan(in_object.pose.orientation.z) ||
        std::isnan(in_object.pose.orientation.w) || std::isnan(in_object.pose.position.x) ||
...
void VisualizeDetectedObjects::visualizeDetectedObjs(
    const autoware_msgs::DetectedObjectArray& in_objects,
    visualization_msgs::MarkerArray&          visualization_markers)
{
    visualization_msgs::MarkerArray label_markers, arrow_markers, centroid_markers, polygon_hulls,
        bounding_cubes, bounding_boxes, object_models, polygon_freespace;
    marker_id_ = 0;
