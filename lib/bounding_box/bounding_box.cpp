...
void checkClusterMerge(std_msgs::Header header, size_t in_cluster_id,
                       std::vector<BoundingBoxPtr>& in_clusters,
    ...
    for (size_t i = 0; i < in_clusters.size(); i++) {
        if (i != in_cluster_id && !in_out_visited_clusters[i]) {
            ...
            if (distance <= in_merge_threshold) {
                ...
                // << std::endl;
                checkClusterMerge(header,
                                  i,
                                  in_clusters,
                                  in_out_visited_clusters,
                                  out_merge_indices,
                                  in_merge_threshold);
            }
        }
...
void mergeClusters(std_msgs::Header header, std::vector<BoundingBoxPtr>& in_clusters,
                   std::vector<BoundingBoxPtr>& out_clusters, std::vector<size_t> in_merge_indices,
...
void checkAllForMerge(std_msgs::Header header, std::vector<BoundingBoxPtr>& in_clusters,
                      std::vector<BoundingBoxPtr>& out_clusters, float in_merge_threshold,
                      bool in_estimate_pose_)
{
    ...
    for (size_t i = 0; i < in_clusters.size(); i++) {
        if (!visited_clusters[i]) {
            ...
            checkClusterMerge(
                header, i, in_clusters, visited_clusters, merge_indices, in_merge_threshold);
            mergeClusters(header,
                          in_clusters,
                          out_clusters,
                          merge_indices,
                          current_index++,
                                 ...
                                 autoware_msgs::CloudClusterArray&                  inOutClusters)
{
    ...
    if (Clusters.size() > 0)
        checkAllForMerge(
            header, Clusters, midClusters, cluster_merge_threshold_, in_estimate_pose_);
    else
        midClusters = Clusters;

    if (midClusters.size() > 0)
        checkAllForMerge(
            header, midClusters, finalClusters, cluster_merge_threshold_, in_estimate_pose_);
    else
...
void BoundingBox::ToROSMessage(std_msgs::Header             header,
                               autoware_msgs::CloudCluster& outClusterMessage)
{
