...
//     pcl::PointCloud<pcl::PointXYZI>::Ptr downsampled_cloud_ptr(new
//     pcl::PointCloud<pcl::PointXYZI>); pcl::PointCloud<pcl::PointXYZI>::Ptr noground_cloud_ptr(new
//     pcl::PointCloud<pcl::PointXYZI>); pcl::PointCloud<pcl::PointXYZI>::Ptr ground_cloud_ptr(new
//     pcl::PointCloud<pcl::PointXYZI>); pcl::PointCloud<pcl::PointXYZRGB>::Ptr
//     cluster_cloud_ptr(new pcl::PointCloud<pcl::PointXYZRGB>);
    ...
    const sensor_msgs::PointCloud2ConstPtr& in_sensor_cloud)
{
    std_msgs::Header                        header = in_sensor_cloud->header;
    pcl::PointCloud<pcl::PointXYZI>::Ptr    in_cloud_ptr(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::PointCloud<pcl::PointXYZRGBA>::Ptr clip_cloud_ptr(new pcl::PointCloud<pcl::PointXYZRGBA>);
    pcl::fromROSMsg(*in_sensor_cloud, *in_cloud_ptr);
    // 提取ROI
    int64_t tm0 = GetTime();
    roi_clip_.GetFusionROI(in_cloud_ptr, image, clip_cloud_ptr);
    std::cout << "clip_cloud_ptr " << clip_cloud_ptr->points.size() << std::endl;
    int64_t tm1 = GetTime();
    ROS_INFO("ROI_Clip cost time:%ld ms", (tm1 - tm0) / 1000);

    pcl::PointCloud<pcl::PointXYZRGBA>::Ptr downsampled_cloud_ptr(
        new pcl::PointCloud<pcl::PointXYZRGBA>);
    pcl::PointCloud<pcl::PointXYZRGBA>::Ptr noground_cloud_ptr(
        new pcl::PointCloud<pcl::PointXYZRGBA>);
    pcl::PointCloud<pcl::PointXYZRGBA>::Ptr ground_cloud_ptr(
        new pcl::PointCloud<pcl::PointXYZRGBA>);

    // 地面分割
    double time_taken;
    patch_work_.estimate_ground(
        *clip_cloud_ptr, *ground_cloud_ptr, *noground_cloud_ptr, time_taken);
    int64_t tm2 = GetTime();
    ROS_INFO("remove ground cost time:%ld ms", (tm2 - tm1) / 1000);
    PublishCloud(&pub_clip_cloud_, in_sensor_cloud->header, clip_cloud_ptr);
    PublishCloud(&pub_noground_cloud_, in_sensor_cloud->header, noground_cloud_ptr);
